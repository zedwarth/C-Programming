...
void textFile( FILE *readPtr )
{
	...
	if ( ( writePtr = fopen( "accounts.txt", "w" ) ) == NULL ) {
		...
		/* copy all records from random-access files into text files */
		while ( !feof( readPtr ) ) {
...
void updateRecord( FILE *fPtr )
{
	...
	if ( client.acctNum == 0 ) {
		...
		/* request transation amount from user */
...
void deleteRecord( FILE *fPtr )
{
	struct clientData client; /* stores reocrd read from file */
...
void newRecord( FILE *fPtr )
{
	/* creat clientData with default information */
	struct clientData client = { 0, "", "", 0.0 };
	...
	if ( client.acctNum != 0 ) {
		printf( "Account #Account #%d already contains information.\n",
			client.acctNum );
	} /* end if */
	else { /* create record */
