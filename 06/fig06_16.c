...
void mode( int freq[], const int answer[] )
{
	...
	/* output header for results colums */
	printf( "%s%11s%19s\n\n%54s\n%54s\n\n",
	...
	for ( rating = 1; rating <= 9; rating++ ) {
		...
		/* keep track of mode value and largest frequency alue */
...
void printArray( const int a[] )
{
	...
	for ( j = 0; j < SIZE; j++ ) {
		...
		if ( j % 20 == 0 ) { /* begin new line ever 20 values */
			printf( "\n" );
		} /* end if */
